...
* Search for: `'thehonestdog'` and replace with: `'project-name'` (inside single quotations) to capture the text domain
* Search for: `thehonestdog_` and replace with: `project-name_` to capture all the function names
* Search for: `Text Domain: thehonestdog` and replace with: `Text Domain: project-name` in style.css
* Search for (and include the leading space): <code>&nbsp;_s</code> and replace with: <code>&nbsp;Project Name</code> (with a space before it) to capture DocBlocks
* Search for: `thehonestdog-` and replace with: `project-name-` to capture prefixed handles
* Search for `thehonestdog.pot` and replace with: `project-name.pot` to capture translation files
